 ...
 * Each log entry updates a byte in the cache.
 *
 * During reads:
 ...
 *******************************************************************************/
uint8_t eeprom_read_byte(const uint8_t *Address) { return EEPROM_ReadDataByte((const uint32_t)Address); }

void eeprom_write_byte(uint8_t *Address, uint8_t Value) { EEPROM_WriteDataByte((uint32_t)Address, Value); }

void eeprom_update_byte(uint8_t *Address, uint8_t Value) { EEPROM_WriteDataByte((uint32_t)Address, Value); }

uint16_t eeprom_read_word(const uint16_t *Address) { return EEPROM_ReadDataWord((const uint32_t)Address); }

void eeprom_write_word(uint16_t *Address, uint16_t Value) { EEPROM_WriteDataWord((uint32_t)Address, Value); }

void eeprom_update_word(uint16_t *Address, uint16_t Value) { EEPROM_WriteDataWord((uint32_t)Address, Value); }
...
void eeprom_write_dword(uint16_t *Address, uint32_t Value) { EEPROM_WriteDataDWord((uint32_t)Address, Value); }

void eeprom_update_dword(uint16_t *Address, uint32_t Value) { eeprom_write_dword(Address, Value); }
...
void eeprom_write_block(const void *buf, void *addr, size_t len) {
    ...
    if ((uint32_t)dest % 4 == 0) {
        ...
        while (len > 1) {
            ...
            eeprom_write_dword((uint16_t *)dest, dwvalue);
            dest += 4;
            src += 4;
            len -= 4;
        }
    ...
    if ((uint32_t)dest % 2 == 0) {
        uint16_t wvalue;
        bool     waligned = ((uint32_t)src % 2 == 0);
        while (len > 1) {
            if (waligned) {
                wvalue = *(uint16_t *)src;
            } else {
            ...
            eeprom_write_word((uint16_t *)dest, wvalue);
            dest += 2;
            src += 2;
            len -= 2;
        }
