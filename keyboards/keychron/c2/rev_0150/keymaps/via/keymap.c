...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_ansi_104(
        ...
        KC_GRV,   KC_1,     KC_2,     KC_3,     KC_4,     KC_5,     KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,  KC_EQL,   KC_BSPC,  KC_INS,   KC_HOME,   KC_PGUP,  KC_CLR,  KC_PSLS,  KC_PAST,  KC_PMNS,
        KC_TAB,   KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,     KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,     KC_LBRC,  KC_RBRC,  KC_BSLS,  KC_DEL,   KC_END,    KC_PGDN,  KC_P7,   KC_P8,    KC_P9,
        KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,            KC_ENT,                                  KC_P4,   KC_P5,    KC_P6,    KC_PPLS,
        KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,            KC_RSFT,            KC_UP,               KC_P1,   KC_P2,    KC_P3,
        KC_LCTL,  KC_LOPT,  KC_LCMD,                                KC_SPC,                                 KC_LCMD,  KC_ROPT, MO(MAC_FN),KC_RCTL,  KC_LEFT,  KC_DOWN,   KC_RGHT,  KC_P0,             KC_PDOT,  KC_PEQL),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case KC_SIRI:
            if (record->event.pressed) {
                SEND_STRING(SS_DOWN(X_LGUI) SS_DOWN(X_SPACE) SS_DELAY(500) SS_UP(X_LGUI) SS_UP(X_SPACE));
            } else {
...
void rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    if (default_layer_state == MAC_BASE) {
        RGB_MATRIX_INDICATOR_SET_COLOR(106, 0, 0, 255);
        RGB_MATRIX_INDICATOR_SET_COLOR(107, 0, 0, 0);
    } else {
        RGB_MATRIX_INDICATOR_SET_COLOR(107, 0, 0, 255);
        RGB_MATRIX_INDICATOR_SET_COLOR(106, 0, 0, 0);
    }
